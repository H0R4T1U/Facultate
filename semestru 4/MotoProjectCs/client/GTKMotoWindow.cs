...
namespace MotoClientGTK
{
    public class MotoWindow : Window
    {
        ...
        private async void PopulateRaceList()
        {
            try
            {
                log.Debug("PopulateRaceList");
                raceStore.Clear();
                var races = await ctrl.GetAllRaces();
        ...
        private async void HandleSearchTeam(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Player[] players = await ctrl.SearchTeam(teamName, raceId);
                playerStore.Clear();
